...
func TestRenderToolBlocksNoWrap(t *testing.T) {
	...
	res := agent.ToolResult{Meta: &shell.Result{
		Stdout:   []shell.Chunk{{Data: strings.Repeat("输出内容宽字符测试", 30) + "\n"}},
		Duration: 250 * time.Millisecond,
		ExitCode: 0,
	}}
...
func TestToolViewInteractive(t *testing.T) {
	...
	time.Sleep(30 * time.Millisecond)
	view.Handle(agent.Event{Kind: agent.EventToolEnd, ToolName: "run_shell", ToolArgs: `{"command":"sudo -S true"}`, Interactive: true, Result: agent.ToolResult{Meta: &shell.Result{Command: "sudo -S true", ExitCode: 1}}})
	out := buf.String()
...
func TestToolViewNonInteractive(t *testing.T) {
	...
	time.Sleep(30 * time.Millisecond)
	view.Handle(agent.Event{Kind: agent.EventToolEnd, ToolName: "run_shell", ToolArgs: `{"command":"echo hi"}`, Result: agent.ToolResult{Meta: &shell.Result{Command: "echo hi", ExitCode: 0}}})
	out := buf.String()
	if out == "" {
		t.Fatal("输出为空则负向断言会静默通过")
	}
