...
3) 状态行追加语义：script -qec "./tanya" /dev/null，跑一条 sleep 30 的命令，看 `▸ 工具名` 只出现一次、
   ...
   含思维链的模型在思维链显示关闭时另有 `» 等待响应 0s ` → `» 思考中 3s ` 的相位切换（秒数连续、每次请求只多一行）
...
7) 思维链显示：`/reasoning on` 后提问，思维链以 markdown 呈现并夹在 `─── 思考 ───` / `─── 思考结束 · 0.4s ───` 之间（Think 色，无 `» 思考中` 行），
   正文紧随下分隔符；`/reasoning` 显示 `思维链显示: 开`，`/reasoning off` 后回到 `» 思考中` 相位；思维链里贴未闭合 ``` 的畸形输出不得吞掉后续正文
TIP
