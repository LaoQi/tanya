...
- REPL 输入三路分发（`repl/shellrun.go`）：`/` 白名单斜杠命令（控制面）、`:`/`：` 与 LLM 对话、其余在本目录直通 shell；执行面 `exit`/`quit` 内建，目录命令走**可剥离切面** `repl/localcmd.go`（`tryLocalCommand` 单入口：`cd`/`pushd`/`popd` 一律拦截告警、不执行；工作目录固定为启动目录，REPL 不 `os.Chdir`、不设 `cmd.Dir`，剥离步骤见 design.md），命令不进 LLM 上下文；直通期间抑制 `^C` 只能用 `signal.Notify`——`signal.Ignore` 的忽略处置会被 exec 继承，命令自身也收不到 `^C`
