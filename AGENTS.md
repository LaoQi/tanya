...
- 中断依赖两项终端不变量（`ISIG` 开启、前台组是 tanya），自愈时机与信号退出码口径（`128 + signum`）见 `docs/ctty.md`；`/dev/tty`、前台组、termios 读写与模式复位、终端探测（isatty/尺寸/VT）等原语一律走 `ctty`，是否移交/夺回/复原由 `agent`、`readline` 各自决定：`run_shell` 移交终端前快照 termios、子进程结束后（含超时强杀）复原 termios，并在自己仍是终端前台（`ctty.IsForeground`）时发屏幕模式复位（`ctty.ResetModes`，前台路径与 pty 桥接 release 都发；属性类复位（SGR/字符集/模式）一律排在 `DECSC` 之前，会移光标的 `DECRST 1049` 与 `CSI r` 包在 `DECSC`…`DECRC` 内且顺序不可调换，避免后续输出从屏幕顶部开始画），自愈侧把被外部留成 raw 的终端拉回 canonical
