...
- 中断依赖两项终端不变量（`ISIG` 开启、前台组是 tanyan），自愈时机与信号退出码口径（`128 + signum`）见 `docs/ctty.md`；`/dev/tty`、前台组、termios 读写与模式复位等原语一律走 `ctty`，是否移交/夺回/复原由 `agent`、`readline` 各自决定：`run_shell` 移交终端前快照 termios、子进程结束后（含超时强杀）复原并 `ResetModes`，自愈侧把被外部留成 raw 的终端拉回 canonical
