...
func TestPlatformWindowsCapabilities(t *testing.T) {
	ps := platform.Capabilities(&profile{Name: "pwsh", Kind: KindPowerShell})
	...
	cmd := platform.Capabilities(&profile{Name: "cmd", Kind: KindCmd})
...
func TestPlatformWindowsDescGolden(t *testing.T) {
	...
	got := describeShell(platform, &profile{Name: "pwsh", Kind: KindPowerShell}, []string{"git", "where"})
	...
	got = describeShell(platform, &profile{Name: "cmd", Kind: KindCmd}, nil)
