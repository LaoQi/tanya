...
func TestPlatformWindowsDescGolden(t *testing.T) {
	...
	got := describeShell("windows", platform, &shellProfile{Name: "pwsh", Kind: KindPowerShell}, []string{"git", "where"})
	...
	got = describeShell("windows", platform, &shellProfile{Name: "cmd", Kind: KindCmd}, nil)
