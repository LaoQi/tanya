...
- 以 shell 为核心的工具体系：模型可直接执行 shell 命令（自动适配平台：Linux/macOS bash/sh/ash，Windows pwsh/powershell；`shell` 配置可指定任意 shell）；命令前后保存/复原控制终端状态（复原 termios + 在自己是前台时复位屏幕模式：SGR、字符集、滚动区/换行/光标/鼠标等，会移光标的序列包在 DECSC/DECRC 内、属性类复位排在其前，不会把后续输出带到屏幕顶部），被超时强杀的交互程序不会留下坏终端
