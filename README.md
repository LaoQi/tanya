...
- 二次确认：`/archive` 一律先出报告（将归档几个、多大体积，附跳过/失败行）再问 `现在归档？[y/N]`，`y`/`yes` 才落卷、其它输入取消；命令只在完整交互环境（rich 输出且 stdin/stdout 都是终端）可用，`-p`/ask/管道/非终端只提示不可用，没有跳过确认的开关
...
配置 `auto_archive: true`（`auto_archive_threshold` 默认 64、`auto_archive_keep` 默认 16）后，REPL 启动时若当前工作区活跃会话数达到阈值，会在欢迎屏后询问：

```
当前工作区有 68 个活跃会话（阈值 64），建议归档较早的，只保留最近 16 个。
将归档 52 个会话（约 12.3M）。现在归档？[y/N]
```

输入 `y`/`yes` 立即归档（只保留最近 `auto_archive_keep` 个，其余打包成一卷），其它输入跳过。跳过不做记忆，下次启动仍会问，想彻底关掉提示设 `auto_archive: false`。

只走配置文件，没有对应环境变量；仅 REPL 启动会问——`ask` 单发、`-p`、stdout 非终端、无控制终端、`-n`（不落盘）一律不提示（纯文本模式与 stdout 非终端在开终端之前就静默返回，不会把提示写进管道、也不会阻塞在 `/dev/tty` 上）。阈值需 `>= 2`，保留数需 `>= 0` 且小于阈值（`0` = 除当前会话外全部归档），越界会在启动时报错。
...
**标题区随命令长度换形态**：命令短到能与工具名同行时内联一行（`▸ run_shell ls -la`）；放不下或原本多行（heredoc、多行脚本、长管道）时转块形态——首行只有工具名，其后是显式 `cwd` 行（`  cwd: ~/proj`，不缩写）与**按显示宽度折行的命令区**（每行 `  $ ` 前缀，与原输出区的 `  ` / `  2| ` 缩进区分），制表符按 4 空格摊平、空行与缩进照原样保留；命令行数上限 8 行，超出保留头 6 行 + 尾 1 行并提示 `… 省略 N 行（完整命令见 /history）`（`/history n` 里的工具消息保留完整参数）。折行只切分不改写内容、不做词级折行，因此命令一定能看全，且任何一行都不超过终端宽度。
