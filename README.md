...
- 环境段（OS/shell 执行契约，**不含 CWD**）由 `main` 组装，紧随内置提示词之后、两层 AGENTS.md 之前，作为 agent 的初始化基座一次注入；随快照落进会话首行、会话期间不变，模型可据此构造 `run_shell` 命令（工作区路径按需 `pwd` 自探）
