...
桥接期间真实 tty 归 bridge，repl 侧不得写入。现状 `interactive` 已停 spinner；标题行在切 raw **之前**打印，结果块在 `stop()` 之后追加渲染。
...
| 桥接期 repl 侧写终端互斥 | **复核无问题** | spinner 已停、标题行在切 raw 前打印、结果块在 `stop()` 后渲染；ask 模式无其它写终端路径 |
