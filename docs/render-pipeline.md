...
表格是「闭合才确定」的块级结构（列数、对齐、列宽），而 REPL 不做回退重绘——已上屏的行不能重画。落地取**前瞻固定三行 + 尽力而为**：

- **IR 新增 `Table` 块**（`Aligns []Align` / `Widths []int` / `Rows []TableRow{Cells,Header}` / `Top`、`Bottom`、`Compact`）。列宽随块携带，渲染器保持无状态值类型；`Top`/`Bottom` 只是块级边线标记，类型上可表达整表，管线实际始终逐块输出（`/history` 回放与实时正文走同一 `Write`/`Close` 逐块序列，拼接结果一致；`ask` 是旁路，不进本管线）
- **前瞻时序**：含竖线的行先只作**候选表头**暂存（不出块，普通正文零延迟）；下一行匹配 GFM 分隔行且列数一致才进表格态，否则候选行**回退为 `Paragraph`**、本行照常解析。首数据行到达时列宽（逐列 `max(表头, 首行)` 的可见宽度）才确定，上屏「上框 + 表头 + 中分隔线 + 该行」一个块，此后逐行出块，空行/非表格行/`Close()` 补下框。只有表头无数据行的表在收尾时补全四行。表格态不累积内存，看门狗语义不变（`flushPending` 先结算表格）
- **对齐**：`:--` 左、`--:` 右、`:-:` 中，缺省左；行内标记按**可见宽度**计（`**bold**` 的列宽是 4 不是 8），填充空格走显示宽度
- **外框**：`┌┬┐├┼┤└┴┘─│`（`render/theme` 只管颜色：`TableHead` 亮青加粗、`TableBorder` 暗灰，字符集不进 theme），单元格两侧各一格内边距
- **列宽不封顶、内容不截断**（已决取舍）：首数据行之后的更宽单元格原样输出，该行超出既定宽度、边线错位——内容优先于整齐；单列上限不做
- **终端宽度只用于切紧边距**（`MarkdownBuf.SetWidth`，`flow`/`mdBlocks` 由 `r.view.width()` 注入）：松边距行宽超终端宽时改 `Compact`（去掉内边距），仍超宽则照原样渲染，不折行不截断。`NO_COLOR` 等无色 TTY 仍画框线（去色不去结构），非 TTY 与 plain 档照旧走旁路不渲染
- **边界**：`\|` 转义为字面竖线、首尾竖线可省、缺列补空、多列丢弃；引用/列表前缀行不作表格候选（不抢分组语义）；单元格不折行、不支持嵌套块

### 范围

- 第一批：块级四样（代码块/标题/列表/引用）+ 行内三样（粗体/斜体/行内码）
- 第二批（2026-09-20 落地）：表格（见上）
- 远期：truecolor palette、非终端 Renderer
...
3. **IR 无布局**——不含宽度、换行点、对齐；折行永远是渲染时行为（resize 后下次渲染自动用新宽度）
   - **落地修订（2026-09-20）**：`Table` 是本条的唯一例外——它携带 `Widths`/`Aligns`。理由是流式不可回退：列宽必须在解析期一次定死（前三行），渲染期已无信息可算；渲染器仍不持有任何跨调用状态，`Widths` 只是「已定布局的渲染指令」
...
- ~~**不做布局指令**（对齐/列宽）~~ **已修订（2026-09-20）**：表格落地后新增 `Table` 一类块并携带 `Aligns`/`Widths`，见 §10 与 §13.3；现有七类块未动，其余来源仍不做布局指令
...
- **阶段 4 之表格（2026-09-20 落地）**：IR `Table` + markdown 表格状态机 + 渲染器表格分支 + `MarkdownBuf.SetWidth` 接线，见 §10

验证：`go build/vet/test/-race ./...`，每阶段加 pty 人工冒烟（REPL 提示符、工具块、/load 菜单、spinner）。

**落地状态**：阶段 1–3 已实施（含包结构从单包 `style` 改为 `render` 树的大拆分，见 `docs/style-split.md`）；阶段 4 中**表格已实施**（2026-09-20，§10），truecolor palette 已在拆包时明确删除（§4），非终端 Renderer 保持远期。
