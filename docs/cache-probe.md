...
tanya 的 history 全程 append-only、system 前缀冻结、工具描述按请求组装，这些设计都建立在"对端有前缀缓存"这一假设上。其中"system 前缀冻结"是**需要代码维护的不变量**：规则快照在 `/new`/`/load` 冻结，环境段自 2026-09-14 起在 `agent.New` 装配期定格（`/switch` 换工作区时随新工作区重建）（此前 `WORKSPACE` 行每轮重探、是 system 内唯一会自行变化的内容，见《落实：env 段的动态源》）。本文记录该假设的实测验证结论，以及"改动 prompt 中的哪一部分会损失多少缓存"的量化台阶，供后续设计（动态工具描述、运行时改配置、会话注入）引用。该不变量的**适用范围**（同一二进制内保证、跨版本允许变化）见 `docs/design.md`《系统提示与缓存友好》。
...
- **工具顺序必须稳定**：交换工具顺序会在 tools 段开头断开（实测命中 5248 → 5120）。装配点 `tools.Standard` 的固定顺序（`run_shell` → `get_time` → `get_env` → `calc`）+ `agent` 恒末位追加 `agent_custom` 满足（2026-09-25 工具外置后清单字节与旧的 `allTools()` 一致）；条件注册（如 `run_shell` 缺失时）会从变化点起失效，影响面限于 tools 段。
