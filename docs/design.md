...
tools/             package tools：外置工具集（根包 tools.Standard 装配标准集与顺序）+ tools/shell（run_shell：profile 解析、前台/桥接执行、平台分片）+ tools/builtin（get_time/get_env/calc）
...
- **进程事实**：`cwd`（启动目录，全程不 `os.Chdir`）；家目录与工作区基准在 `main` 装配一次、注入 `tools/shell`（`tools.Options.Home` / `Workspace func() string`，见 `docs/shell-tool.md`）；`/switch` 换工作区时由 `Agent.loadWorkspace` 重建派生态（system 提示、会话存储），进程 cwd 不变，**注入的工具不重建**——`run_shell` 的默认目录经 `Workspace` 回调活取 `Agent.Workspace()`，故换区自动跟随
...
### run_shell（`tools/shell/tool.go` 组件 + `tools/shell/shell.go` 叶子）

- 参数：`command`（必填）、`cwd`（可选，命令执行目录，默认当前工作区）、`timeout`（默认 60s，上限 900s）、`interactive`（布尔，默认 false）
- 执行目录：默认取**当前工作区**（`tools.Options.Workspace func() string` 每次 `Invoke` 现取，`main` 传 `func() string { return a.Workspace() }`，故 `/switch` 后自动跟随；回调为 nil 或返回空串时退回进程 cwd），`cwd` 参数是单次覆盖——显式时设 `cmd.Dir`（不改进程 cwd），解析规则为 `~`/`~/x` 展开家目录、相对路径按工作区基准合成（家目录由 `tools.Options.Home` 注入；缺基准时相对路径直接失败而非退回环境 cwd），随后 `os.Stat` 校验——不存在或非目录直接快速失败（`MsgBadCwd`，不启动进程）。只有显式指定才回填 `Result.Cwd`（绝对路径）并让 `String()` 首行输出 `cwd: <路径>`，默认目录不改变输出形状。桥接与回退两条路径均生效（`Bridge.Prepare` 只改 `SysProcAttr`/标准流/`Env`，不覆盖 `cmd.Dir`）
- 波浪号边界（不宣传的默认契约）：只处理 `~` 与 `~/x`（`~` 展开家目录，基准由 `tools.Options.Home` 注入）；`~user` 与 Windows 风格 `~\x` 一律不展开——按相对路径解析并因不存在直接报 `MsgBadCwd`（快速失败，不误执行）。`run_shell` 工具描述与 `cwd` 参数描述均不提 `~`（避免引导模型使用）；env 段 `CWD:` 行的 `~/...` 只是完整路径的显示缩写，不是路径语法引导
...
- 平台抽象（`tools/shell/platform.go`，无 tag）：`shellPlatform{GOOS, Candidates, ConfigureGroup, KillGroup, ProtectSignals, ExitCode, ProcessStopped, Programs, Capabilities, DecodeOutput}` 一张表承载全部平台差异（候选链、进程/信号/退出码语义、可用程序清单与工具描述的能力句——run_shell 描述随平台切换；Windows 清单同时含 coreutils/Git for Windows/msys2 可能提供的 unix 工具与 Windows 原生程序，仍是探测到才列出，不会暗示不存在的工具链），按可实现目标平台一文件装配——`platform_posix.go`（`linux || darwin`）承载共享实现（候选链、程序清单、能力句、进程组、信号防护、退出码），`platform_linux.go`（`linux`，`/proc` 挂起探测）与 `platform_darwin.go`（`darwin`，`sysctl` 挂起探测）各自装配表与平台差异、`platform_windows.go`（`windows`）、`platform_stub.go`（其余平台，仅保证可编译）；各分片一律经无 tag 文件的 `fillDefaults` 装配，nil 函数字段落到安全默认（no-op 进程组配置、默认杀进程、默认退出码、恒 false 挂起探测、空能力句、恒等 `DecodeOutput`——windows 分片填入代码页兜底转码，见《输出捕获》），`GOOS` 空则填 `runtime.GOOS`（平台名的唯一引用点，分片不再各自声明），漏字段不再等到调用点才崩；`ProtectTerminalSignals`（`sync.Once` 包装，导出供 `main` 启动时调用）同在无 tag 文件，`TestPlatformComplete` 继续守卫候选链非空与程序清单无空串（这两个字段不做兜底：候选链空即启动报错，程序清单空即无清单）。取代原先按方法散落的 `shell_unix`/`shell_other`/`shell_proc_*`/`shell_candidates_*` 七文件与三套 tag 口径
- 工具描述拼装（`Tool.toolDesc` → `describeShell(plat, profile, programs)`，与平台无关、可注入任意平台值）：平台名取自 `plat.GOOS`（不再单独传 `runtime.GOOS`，与能力句、程序清单同源同一张表；`TestPlatformComplete` 断言其等于运行时 GOOS）：`在 <GOOS> <name> 中执行命令（<语法提示>），返回 stdout/stderr/退出码。` + cwd 契约句（当前工作区执行、无需 `cd`、需要其它目录用 `cwd` 参数，2026-09-16 前移至能力句之前） + 平台能力句（`platform.Capabilities`，按平台与 `profile.Kind` 给差异：posix 的文本工具链/`/dev/tty` 交互提示/`128+signum` 退出码；windows 的 PowerShell 对象管道或 cmd 内建/无 `/dev/tty`/unix 工具链以「可用程序」清单为准——coreutils、Git for Windows 或 msys2 装上就有，探测不到就不提）+ 用途句（系统操作首选）+ `可用程序: ...`
- shell 解析（`shell.New`，`tools.Standard` 构造时一次性解析并定格）：
  ...
  - 全部落空（含配置的 shell 不存在）：解析返回错误（`MsgNoShellFmt`/`MsgShellOverrideFmt`，含候选清单与配置提示），`tools.Standard` 立即透传，`main.go` 打印后以 1 退出——无降级路径，`Tool.profile` 在其后恒非 nil，profile 的非空成为不变量（`main` 的装配失败即退出，故 `run_shell` 在 `main` 路径上恒定注册、env 段恒定输出 SHELL/TIMEOUT/OUTPUT 行）
...
- 组件化（`docs/shell-tool.md`）：`Tool` 是 shell 执行层唯一所有者，`profile`/`programs`/`home`/`bridge` 在装配期定格、之后只读（`workspace` 是活取回调，只读的是「怎么取」而非取到的值），`run` 每调用状态全在栈上（可重入）；唯一可变字段是终端租约 `ttyMu`——真实终端进程内只有一份，桥接与前台移交两条路径都在锁内。组件内不读环境（无 `os.Getwd`/`os.UserHomeDir`/`exec.LookPath`/`runtime.GOOS`，`LookPath` 可按需注入以便测试），装配点（`tools.Standard`）各读一次注入。包级可变状态（`shellRuntime*`/`shellLookPath`/`ttyBridgeMu`+`ttyBridgeCur`）已删除；`describeShell`/`runShellParams` 为纯函数（组件内不读 GOOS：平台名来自构造期定格的平台表）；工具清单由 `tools.Standard`（顺序契约）与 `assembleTools`（末位追加）显式组装、经 `toolRegistry.defs()` 在 `NewClient` 构造期注入 client（请求组装不再伸手读包级清单）
...
- 字段集：`Result` 为 Command/Cwd/Stdout/Stderr chunks/Err/ExitCode/TimedOut/Interrupted/Stopped/NotStarted/Duration
...
- **`run_shell`（`shellArgsView`）**：`command` 短到能与工具名同行时**内联单行**（`▸ run_shell ls -la`，与旧版逐字节一致）；放不下或原本多行时转**块形态**——首行只有工具名（`▸ run_shell`），其后是显式参数行与折行的命令区（每行 `  $ ` 前缀，与输出区的 `  ` / `  2| ` 缩进区分）：`cwd` 行（`  cwd: <原样值>`，不缩写）与 `timeout` 行（`  timeout: 90s`，`timeout > 0` 才显示，字面秒数）各占一行，任一参数存在即转块形态；`interactive` 不显示（交互引导行已提示）；JSON 解析失败退回原样展示参数文本（`  $ ` 前缀）；`command` 为空时不产生命令区（仅显式参数行）
- **其余工具（`genericArgsView`）**：通用键值渲染——内联 `▸ calc expression: (1+2)*3/4`，多参数用 ` · ` 连接（`▸ agent_custom action: set · key: model · value: gpt-5`）；放不下或多行时转块形态，每项一行 `  key: value` 并按宽度折行。键序按 JSON 原文顺序（`json.Decoder.Token` 流式读，`Unmarshal` 到 map 会按字母序重排，模型给的 schema 顺序更可读）。值渲染（`argDisplayValue`）：字符串原样（含多行、制表符摊平 4 空格）、空串显式 `""`、标准数组逗号连接（`HOME, PATH`；含对象/嵌套数组时退回紧凑 JSON）、对象与嵌套结构压成紧凑 JSON、`null` 显式写出、数字与布尔按原文；参数为空（`{}` 或无键）只显示工具名，坏 JSON 退回原样展示。省略文案分家：shell 用 `MsgCmdOmittedFmt`（`… 省略 N 行（完整命令见 /history）`）、通用用 `MsgArgsOmittedFmt`（`… 省略 N 行（完整参数见 /history）`）
- **折行与上限（`commandLines` / `capLines` + `render/term.Wrap`）**：两者共用 `capLines`——保留原有的换行结构与行首缩进（不再用 `; ` 压成单行——旧实现会把 `for …; do` / `if …; then` 拼成语法上不存在的 `do; if`，且丢空行与缩进）；制表符先按 4 空格摊平（`runeWidth` 把 `\t` 当单列、与终端制表位不符，不摊平则折行位置与显示不符）；`Wrap` 按显示宽度切分（宽字符整字换行、遇 `\n` 硬断行、`\r` 丢弃），只切分不改写内容、**不做词级折行**（超长 token 一样硬切）。行数上限 `toolCommandMaxLines = 8`：超出保留头 6 行 + 省略行 + 尾 1 行——有序脚本（命令）省略中段比省略尾部更不易误读收尾的 `done`/`EOF`，参数同理；完整内容始终在 `/history n` 的工具消息里（显示侧改造不影响模型通道与会话存储）
- 参数文本里的 ANSI 由调用点的 `Dim.Frame` 清洗（`Wrap` 遇到序列原样保留、不计宽度），折行发生在清洗之前，故宽度计算不会被模型可控的转义序列干扰
...
免确认轻量工具，`Tools()` 表驱动返回 `[]agent.Tool`（经 `agent.NewTool` 构造，描述与参数随实现同处）：
...
- `/load` 无参打开方向键选择菜单（`repl/picker.go`，stdin 非终端降级为序号输入），候选 Display 带时间/条数/简介；**按屏幕高度分窗**：窗口项数 = `rows-2`（标题占一行，末尾再留一行余量——末项以 `\r\n` 收尾，块若占满整屏会触发滚屏、使上移记账失效），每次重绘的上移量按上次实际写入的行数记账（`p.lines`）而非候选总数，光标顶出窗口边界才平移窗口（不重新居中），每行按 `cols-1` 截断（含标题），标题带 `（选中序号/总数）`；`Size()` 不可得（如 `Degraded`）时退回全量渲染，回合中尺寸变化则放弃上移锚点另起一块重画
...
- tty 桥接（`bridge.go` 接口 + `bridge_linux.go` 实现 + `bridge_stub.go` 非 Linux 返回 `ErrUnsupported`）：为 `interactive: true` 的 run_shell 提供"命令在自己的 pty 中运行"的执行器（`TTYBridge.Prepare/Attach`），复用本包 termios 读写与 raw 语义；生产入口 `readline.NewTTYBridge()` 由 `main.go` 填进 `tools.Options.Bridge`、`tools.Standard` 构造期定格进 `shell.Tool` 的 bridge 字段（`shell.Bridge` 接口与 `readline.TTYBridge` 方法集一致，直接赋值即可），测试可在 `shell.Config.Bridge` 里给 fake bridge
...
标准库 `testing` + `httptest` mock LLM（`agent/mock_test.go`，脚本化 `mockStep`，content/arguments 多 chunk 发送以覆盖流式合并）。覆盖 calc（`tools/builtin`）/shell（`tools/shell`）/config/SSE 解析/trim/会话往返/Ask 全链路/回调。readline 用 fakeTerm 注入按键，终端挂断/空闲超时/转义残片等真实 pty 行为由自驱动 pty 自动覆盖（`readline/terminal_posix_pty_test.go`），其余真实终端行为 pty 人工验证。repl 覆盖渲染纯函数与非 TTY 降级。
...
pty 桥接三层测试：① `readline/bridge_linux_test.go` 自驱动集成（测试自身分配 pty 充当真实 tty，经 `newBridgeTTY` 注入）断言子进程 `/dev/tty` 可读、`tty` 输出为 pty slave、`GPG_TTY` 覆盖、初始尺寸复制、raw 设置与恢复、子进程退出后 master 收到 EIO（防忘关 slave）、Attach 前预置输入不丢、子进程存活时 `stop()` 及时返回；② `tools/shell/bridge_test.go` 用 fake bridge（os.Pipe 造流）断言桥接全流程、Prepare/Attach 失败回退现状路径、非交互不触桥接；③ 真实 tty E2E（gated，用 `script -qec` 驱动真实 /dev/tty，不参与默认 `go test`）：`TTY_BRIDGE_E2E=1`（readline 单命令）、`TTY_E2E=1`（agent + tools/shell 全链路，跑 `./tools/shell`）、`TTY_E2E_REUSE=1`（同进程连续两次交互命令，覆盖 `ownTTY` 打开/恢复/重开复用路径）。
...
- 组装：**由 `main` 组装**——`main.systemBase(inv)` = `systemPromptFile`（内置提示词原文）+ 空行 + `envSection(inv)`（`inv` 为 `shell.Resolve(cfg.Shell)` 的结果），经 `agent.WithSystemPrompt` 注入为 agent 的**基座**；`agent` 侧无 env 段概念（无 `Agent.env` / `runtimePrompt` / `envSection` / `EnvReporter`），`promptBuilder.build()` 只在基座后追加两层 AGENTS.md
...
- 事实源单一：OS 行取 `runtime.GOOS`/`GOARCH`（main）；SHELL 行取 `shell.Invocation.Name`（与 `run_shell` 工具描述同源，只报 shell 名、不描述调用形态）；TIMEOUT/OUTPUT 两行由 `tools/shell` 导出常量（`TimeoutSec`/`InteractiveTimeoutSec`/`TimeoutLimit`/`MaxOutput`）程序化生成；TTY 行固定契约文案，无第二份硬编码描述
